...
// its context. Output is markdown grouped by topic.
const EquationsSystemPrompt = `You are a study-aid generator for a Canadian distillery diploma
...
memorise for the exam. Group them by topic. For each one, write:
  ...
  1. The equation as plain text (e.g. "LAA = V × ABV / 100") — no
...
Strict rules:
  ...
  - Skip examples ("a batch with 50 L at 60% ABV"). Equations only.
  - If a relationship is conditional (e.g. only at 15.6°C reference
    temperature), state the condition explicitly.
...
func ExtractEquations(ctx context.Context, c *LLMClient, lessons *ProcessedLessons) (string, error) {
	...
	return c.Chat(ctx, EquationsSystemPrompt, corpus, ChatOpts{
		Temperature: 0.2,
		MaxTokens:   16384,
	})
}
